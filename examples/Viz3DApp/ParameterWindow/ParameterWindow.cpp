...
ParameterWindow::keyPressed(const juce::KeyPress& key, juce::Component* originatingComponent)
{
    ...
    //
    //        if (key == juce::KeyPress::createFromDescription("CTRL + S")) {
    //            auto fileChooser =
    //              juce::FileChooser("Save file", juce::File::getSpecialLocation(juce::File::userHomeDirectory), "*.json");
    //
    ...
    //                        for (const auto& parameter: section.second) {
    //                            if (parameter.second.type == ParameterType::SLIDER) {
    //                                j[parameter.first] = getSliderValue(m_parameters, parameter.first);
    //                            } else if (parameter.second.type == ParameterType::COLOR) {
    //                                j[parameter.first + "X"] = getColorValue(m_parameters, parameter.first).x;
    //                                j[parameter.first + "Y"] = getColorValue(m_parameters, parameter.first).y;
    //                                j[parameter.first + "Z"] = getColorValue(m_parameters, parameter.first).z;
    //                            } else if (parameter.second.type == ParameterType::TOGGLE) {
    //                                j[parameter.first] = getToggleValue(m_parameters, parameter.first);
    //                            } else if (parameter.second.type == ParameterType::COMBO_BOX) {
    //                                j[parameter.first] = getComboBoxValue(m_parameters, parameter.first);
    //                            }
    //                        }
    //                    }
    //
    //                    std::ofstream o{ file.getFullPathName().toStdString() };
    //                    o << j << std::endl;
    //                } catch (...) {
    //                }
    //            }
    //        }
    //        else if (key == juce::KeyPress::createFromDescription("CTRL + O")) {
    //            auto fileChooser =
    //              juce::FileChooser("Open file", juce::File::getSpecialLocation(juce::File::userHomeDirectory), "*.json");
    //
    ...
    //                    for (const auto& section : PARAMETER_ORDERED_DISPLAY_NAMES) {
    //                        for (const auto& parameter: section.second) {
    //                            if (parameter.second.type == ParameterType::COLOR) {
    //                                if (!j.contains(parameter.first + "X")) {
    //                                    continue;
    //                                }
    //
    //                                glm::vec3 color_value{j[parameter.first + "X"], j[parameter.first + "Y"], j[parameter.first + "Z"]};
    //
    //                                setColorValue(m_parameters, parameter.first, color_value);
    //                            }
    //
    //                            if (!j.contains(parameter.first)) {
    //                                continue;
    //                            }
    //
    //                            if (parameter.second.type == ParameterType::SLIDER) {
    //                                setSliderValue(m_parameters, parameter.first, j[parameter.first]);
    //                            } else if (parameter.second.type == ParameterType::TOGGLE) {
    //                                setToggleValue(m_parameters, parameter.first, j[parameter.first]);
    //                            } else if (parameter.second.type == ParameterType::COMBO_BOX) {
    //                                setComboBoxValue(m_parameters, parameter.first, j[parameter.first]);
    //                            }
    //                        }
    //                    }
    //                } catch (...) {
    //                }
    //
    //                m_parameterWindowComponent->syncAll();
    //            }
    //        }
